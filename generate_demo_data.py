...
freq1 = 2 * math.pi / 20
series1 = 10 * np.sin(freq1 * time) + 50
noise1 = np.random.normal(0, noise_level * 10, num_points)
data.append(series1 + noise1)

# Series 2: Higher frequency sinusoid (period ~10)
freq2 = 2 * math.pi / 10
series2 = 8 * np.sin(freq2 * time) + 100
noise2 = np.random.normal(0, noise_level * 8, num_points)
data.append(series2 + noise2)

# Series 3: Lower frequency with different phase (period ~25)
freq3 = 2 * math.pi / 25
series3 = 12 * np.sin(freq3 * time + math.pi/4) + 150
noise3 = np.random.normal(0, noise_level * 12, num_points)
...
series4 = 15 * modulation * np.sin(carrier_freq * time) + 200
noise4 = np.random.normal(0, noise_level * 15 * 0.75, num_points)
...
for series_idx in range(1, num_series + 1):
    ...
    for idx in outlier_indices:
        data[series_idx][idx] += np.random.choice([-1, 1]) * np.random.uniform(20, 40)
